...
The equalities $x(\zer) = \zer$ and $x(b \vee b') = x(b) \vee x(b')$ are
    ...
    adding $b$ to it and generating an ideal $I'$, and $I'$ will still be
    proper because $\neg b \not\in I$. By maximality of $I$ this is
    ...
    $\one = b \vee \neg b$ in $I$, again contradicting that $I$ is proper. It now
...
X$ such that $x \in U$ and $y \in V$.) Finally,
    ...
    % The necessity is for example proved as
...
\end{proposition}
\begin{proof}
    $\epsilon_X$ is continuous: By \cref{Specbasis} and a general fact about continuous functions, it
    suffices to check that $\epsilon_X^{-1}(U_K)$ is open for every $K \in
    \Clp(X)$.  This set is equal to $K$ by \cref{epseq}, and thus open.
